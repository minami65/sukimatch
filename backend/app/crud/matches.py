...
def get_unread_matches(db: Session, user_id: int):
    matches = (
        ...
        .filter(
            or_(
                (Matches.user1_id == user_id) & (
                    Matches.user1_checked_match == False),
                (Matches.user2_id == user_id) & (
                    Matches.user2_checked_match == False)
            )
        )
        .all()
    )
    ...
    for match in matches:
        ...
        print(
            f"[DEBUG 2] match_id: {match.id} | 相手のID(opponent_id): {opponent_id}")
